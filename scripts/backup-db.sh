#!/usr/bin/env bash
# DB backup script for puku-timetracking
...
FILENAME="puku-backup-${TIMESTAMP}.sql.gz"
...
echo "Backing up database '${DB_NAME}' on ${DB_HOST}:${DB_PORT} → ${OUTPUT_PATH}"
...
if [ "${RETENTION_DAYS}" -gt 0 ]; then
  ...
  find "${OUTPUT_DIR}" -name "puku-backup-*.sql.gz" -type f -mtime "+${RETENTION_DAYS}" -delete
  echo "Pruned."
fi
